...
from bnn_mcmc_examples.datasets.noisy_xor.data1.constants import (
...
for i in range(num_classes):
    ax.plot(
        dataset.x[num_training_samples_cumsum[i]:num_training_samples_cumsum[i+1], 0],
        dataset.x[num_training_samples_cumsum[i]:num_training_samples_cumsum[i+1], 1],
        'o',
        color=cols[i],
        marker='o',
        markersize=3,
