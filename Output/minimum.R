...
make_trace_plots <- function(m, n, p, A, B, rho) {
    ...
    starting = rmvnorm(m, mean = c(mu1, mu2), sigma = sigma) # starting from stationarity
    ...
    plot_name <- paste("MVG_trace_plots_stationary", rho, ".pdf", sep = "_")
...
make_plots <- function(A, B, rho, m, n, rep, plot_step = 50) {
    ...
    filename = paste("MVG_running_stationary_newadjsm", "rho", rho, "m", m, "n", n, ".Rdata", sep = "_")
    load(filename)
    starting_buffer <- 0
    ...
    for (k in 4:4) {
        plot_name <- paste("MVG_running_stationary_newajdsm", "rho", rho, "m", m, "n", n, "type", k, ".pdf", sep = "_")
        ind <- (0:(floor((n - starting_buffer) / plot_step) - 1)) * 5 + k
        x <- (0:(floor((n - starting_buffer) / plot_step) - 1)) * plot_step + plot_step
        pdf(paste(plot_name, sep = "/"), height = 5, width = 8)
...
make_plots <- function(A, B, rho, m, n, rep, plot_step = 50, type) {
    ...
    filename = paste("MVG_CDE_stationary_newadjsm", "rho", rho, "m", m, "n", n, ".Rdata", sep = "_")
    load(filename)
    ...
    if (type == 'coverage') {
        ...
        foo <- paste("MVG_coverage_stationary_newadjsm", "rho", rho, "m", m, ".pdf", sep = "_")
        foo <- paste(foo, sep = "/")
...
m_ <- c(5, 10)
plot_step <- 50
checkpoints <- c(1e2, 5e2, 1e3, 1.5e3, 2e3, 5e3, 1e4, 2e4, n)
p <- 2
...
make_trace_plots <- function(m, n, edge1, edge2, p) {
    ...
    init = initial_stationary(m, A, B, mu)
    lupost = lufac(A, B, mu)
    X <- array(, dim = c(m, n, p))
    for (i in 1:m) {
    ...
    plot_name <- "rosenbrock_stationary_trace_plots.pdf"
    pdf(plot_name, height = 6, width = 6)
...
make_plots <- function(m, n, rep, plot_step = 100) {
    ...
    filename = paste("rosenbrock_running_ESS_stationary_newadjsm", edge1, edge2, "m", m, "n", n, ".Rdata", sep = "_")
    load(filename)
    starting_buffer <- 0
    n <- 2e4
    ...
    plot_name <- paste("rosenbrock_running_ESS_stationary_newadjsm", edge1, edge2, "m", m, "n", n, ".pdf", sep = "_")
    pdf(plot_name, height = 6, width = 6)
    par(mar = c(5, 5, 4, 1) + .1)
...
make_plots <- function(rep, m, n) {
    ...
    filename = paste("rosenbrock_CDE_stationary_newadjsm", edge1, edge2, "m", m, "n", n, ".Rdata", sep = "_")
    load(filename)
    ...
    foo <- paste("rosennbrock_coverage_stationary_newadjsm", edge1, edge2, "n", n, "m", m, ".pdf", sep = "_")
    pdf(foo, height = 11, width = 10)
    grid.table(coverage_data)
    dev.off()
}
...
## TRACE PLOTS
make_trace_plots <- function(component, chain)  {
    plot_name = paste("penguins_trace_", component,".pdf", sep = "")
    pdf(plot_name, height = 6, width = 6)
    plot.ts(chain[,component], xlab = "n", ylab = paste("component ", component))
    dev.off()
}
filename = "chain_m__0_0.csv"
chain = as.matrix(read.csv(filename, header = FALSE))

make_trace_plots(23, chain[1e3:1e5,])
make_trace_plots(15, chain[1e3:1e5,])
...
make_plots <- function(m, n, rep, plot_step = 100) {
    ...
    if (n == 1e5){
        filename = "penguins_running_ESS_stationary_newadjsm_m_5_n_100000_.Rdata"
    }
    load(filename)
    starting_buffer <- 1e3
    ...
    x <- (1:(floor((n - starting_buffer) / plot_step))) * plot_step + starting_buffer
    
    plot_name <- paste("penguins_running", "m", m, "n", n, "plot_step", plot_step, ".pdf", sep = "_")
    pdf( plot_name, height = 6, width = 6)
    par(mar = c(5, 5, 4, 1) + .1)
    plot(x, colMeans(plot_mat_abm[, ind]), type = "l", col = "green", lty = 1, lwd = 2, ylim = range(colMeans(plot_mat_abm[, ind]), colMeans(plot_mat_rbm[, ind])), ylab = expression(hat(ESS)/mn), xlab = "Chain Length", cex.axis = 1.4, cex.lab = 1.4)
    se_abm = sqrt(apply(plot_mat_abm[, ind], 2, var) / dim(plot_mat_abm)[1])
